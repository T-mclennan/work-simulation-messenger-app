// USER:
export const GET_USER = "GET_USER";
export const SET_FETCHING_STATUS = "SET_FETCHING_STATUS";

// CONVERSATION:
export const GET_CONVERSATIONS = "GET_CONVERSATIONS";
export const SET_MESSAGE = "SET_MESSAGE";
export const ADD_ONLINE_USER = "ADD_ONLINE_USER";
export const REMOVE_OFFLINE_USER = "REMOVE_OFFLINE_USER";
export const SET_SEARCHED_USERS = "SET_SEARCHED_USERS";
export const CLEAR_SEARCHED_USERS = "CLEAR_SEARCHED_USERS";
export const ADD_CONVERSATION = "ADD_CONVERSATION";
export const SET_CONVO_AS_SEEN = "SET_CONVO_AS_SEEN";
export const INCREMENT_UNSEEN_COUNT = "INCREMENT_UNSEEN_COUNT";
export const SET_LAST_MESSAGE_READ = "SET_LAST_MESSAGE_READ";
export const TURN_OFF_TYPING_NOTIFICATION = "TURN_OFF_TYPING_NOTIFICATION";
export const TURN_ON_TYPING_NOTIFICATION = "TURN_ON_TYPING_NOTIFICATION";

//ACTIVE CONVERSATION:
export const SET_ACTIVE_CHAT = "SET_ACTIVE_CHAT";

//AUTH:
export const CLEAR_ON_LOGOUT = 'CLEAR_ON_LOGOUT';